...
- Frontend-review remediation of `kanban-flat-material`: added a contrasting cobalt `:focus-within` ring to the search container (the input keeps focus semantics, its redundant outline suppressed so keyboard focus is no longer an invisible near-white-on-near-white double ring); raised every filter chip and Board/List segmented button to a full 44px tap at all viewports; updated `DESIGN.md` to match (44px targets, search focus-within treatment, normal 0.16s transitions distinguished from the 1.4s reduced-motion-gated skeleton pulse, states explicitly marked visual/affordance demonstrations with static drag) and replaced the inaccurate "inherits the source repository's license" claim with the actual status (the repository declares no license — no `LICENSE` file, no `package.json` `license` field). Extended the focused Playwright smoke with the smallest robust assertions that catch undersized segmented controls (≥44px), invisible search keyboard focus (container `outline-style/width`), and no horizontal document overflow in the preview at 375/768/1280.
