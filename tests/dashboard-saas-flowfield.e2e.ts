...
// the focused .seg button has a solid >=3px accent-ink (indigo) outline whose
// extent reaches beyond the .seg container on the sides that matter, proving
// the ring needs overflow:visible to render and is therefore unclipped. Also
...
test('opens the dashboard-saas-flowfield design and its isolated preview states', async ({
	page
}) => {
	// ---- Detail page: identity + exact public summary ----
	await page.goto('/designs/dashboard-saas-flowfield');

	await expect(
		page.getByRole('heading', { name: 'SaaS Analytics · Flow Field', exact: false })
	).toBeVisible();
	await expect(
		page.getByText(
			'A calm SaaS product-analytics dashboard on warm tinted paper with opaque hairline-bordered cards floating over a generative canvas particle flow-field. Particle drift speed subtly scales with the active-users delta; a single restrained indigo accent carries every data-viz stroke and focus ring.',
			{ exact: true }
		)
	).toBeVisible();
	...
	// Demonstrated states (incl. the goal/status indicator)
	...
	await expect(frame.getByText('Q3 MRR goal')).toBeVisible(); // goal/status indicator
	...
	// ------------------------------------------------------------------
	// Direct preview route: flow-field signature + AA contrast table +
	// 44x44 targets + no overflow + reduced motion.
	// ------------------------------------------------------------------
	await page.goto('/designs/dashboard-saas-flowfield/preview');
	await page.setViewportSize({ width: 1280, height: 900 });
	await expect(page.getByText('Aurora', { exact: true })).toBeVisible();

	// ---- Flow-field signature: decorative canvas, aria-hidden, pointer-events:none ----
	const flowSig = await page.evaluate(() => {
		const root = document.querySelector('.dash-root');
		const canvas = document.querySelector('canvas.flow');
		if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLElement)) return null;
		const cs = getComputedStyle(canvas);
		const rootCs = getComputedStyle(root);
		// active-user-delta reactivity hook (data attribute) + documented constant.
		const hook = {
			delta: root.getAttribute('data-active-delta'),
			speed: root.getAttribute('data-flow-speed'),
			canvasHook: canvas.getAttribute('data-flowfield')
		};
		return {
			present: true,
			...
			canvasHook: hook.canvasHook,
			delta: hook.delta,
			speed: hook.speed,
			rootPos: rootCs.position
		};
	});
	expect(flowSig).not.toBeNull();
	expect(flowSig!.ariaHidden, 'canvas aria-hidden').toBe('true');
	expect(flowSig!.pointerEvents, 'canvas pointer-events none').toBe('none');
	expect(flowSig!.position, 'canvas absolutely positioned').toBe('absolute');
	expect(flowSig!.delta, 'active-users delta hook present').toBe('3.1');
	expect(flowSig!.speed, 'flow speed hook present').not.toBeNull();
	expect(flowSig!.canvasHook, 'canvas flowfield marker present').toBeTruthy();

	// The flow-field actually renders particles: after a moment, the canvas
	// carries non-transparent pixels (proves the indigo strokeStyle resolves).
	...
	const canvasDrawn = await page.evaluate(() => {
		...
		if (!width || !height) return { drawn: false, w: width, h: height };
		...
		return { drawn: drawn > 5, w: width, h: height };
	});
	expect(canvasDrawn).not.toBeNull();
	expect(canvasDrawn!.w, 'canvas sized to root width').toBeGreaterThan(0);
	expect(canvasDrawn!.h, 'canvas sized to root height').toBeGreaterThan(0);
	expect(canvasDrawn!.drawn, 'flow-field renders particles (strokeStyle resolves)').toBe(true);

	// Visual signature: warm paper canvas (light, warm hue), opaque panels, no
	// gradient on cards; indigo accent.
	const sig = await page.evaluate(() => {
		...
		const canvasD = rgba(cs('.dash-root')?.backgroundColor || 'rgb(0,0,0)');
		// Warm paper: red and green channels noticeably above blue.
		const warmness = canvasD[0] - canvasD[2];
		return {
			canvasLum: bgLum('.dash-root'),
			canvasWarmness: warmness,
			accent: cs('.dash-root')?.getPropertyValue('--accent') ?? '',
			panelLum: bgLum('.panel'),
			panelBorder: cs('.panel')?.borderWidth ?? '0',
			barBackdrop: cs('.appbar')?.backdropFilter ?? 'none',
	...
	expect(sig!.canvasWarmness, 'canvas is warm-tinted (red > blue)').toBeGreaterThan(4);
	expect(sig!.panelBorder, 'panels have hairline borders (>=1px)').not.toBe('0');
	expect(sig!.barBackdrop, 'no backdrop blur on header').toBe('none');
	...
	// actual opaque parent surface. Cards are opaque over the canvas, so we audit
	// directly against the card surface (the decorative field never sits behind text).
	const contrastResults = await page.evaluate(() => {
		...
		const results: { role: string; ratio: number }[] = [
			...
			{ role: 'goal-value', ratio: ratio('.goal .v', '.mini.goal') },
			{ role: 'goal-of', ratio: ratio('.goal-of', '.mini.goal') },
			{ role: 'goal-pct', ratio: selfRatio('.goal-pct') },
			{ role: 'goal-note', ratio: ratio('.goal-note', '.mini.goal') },
	...
	// 2.4.11/2.4.13). .seg { overflow: hidden } would clip the offset
	// :focus-visible ring (3px + 2px = 5px extent).
	// ------------------------------------------------------------------
	...
	// Reduced motion: canvas hidden entirely; skeleton pulse suppressed; the
	// static dashboard stays complete (header, KPIs, accounts, goal all present).
	await page.emulateMedia({ reducedMotion: 'reduce' });
	const reducedState = await page.evaluate(() => {
		const canvas = document.querySelector('canvas.flow');
		const cs = canvas instanceof HTMLElement ? getComputedStyle(canvas) : null;
		const sk = document.querySelector('.sk');
		const skCs = sk instanceof HTMLElement ? getComputedStyle(sk) : null;
		return {
			canvasDisplay: cs?.display ?? '',
			canvasHidden: cs ? cs.display === 'none' : true,
			skeletonStatic: skCs ? skCs.animationName === 'none' : false
		};
	});
	expect(reducedState.canvasHidden, 'canvas hidden under reduced motion').toBe(true);
	expect(reducedState.skeletonStatic, 'skeleton pulse suppressed under reduced motion').toBe(true);
	// Complete static dashboard remains.
	await expect(page.getByText('Aurora', { exact: true })).toBeVisible();
	await expect(page.getByText('$48,200')).toBeVisible();
	await expect(page.getByText('Northwind Labs')).toBeVisible();
	await expect(page.getByText('Q3 MRR goal')).toBeVisible();
	await expect(page.getByText('Live sync interrupted', { exact: false })).toBeVisible();
	await page.emulateMedia({ reducedMotion: null });

	// No resting box-shadow anywhere (hairline-first, elevation-free at rest).
