...
2. Choose a stable unique slug and create `src/lib/designs/<slug>/`.
...
6. Keep the entry `draft` while building, move it to `reviewed` for quality review, and use `production-ready` only after it meets the [product quality standard](../ui-factory-product-vision.md#quality-standard).
7. Run the checks below and review the rendered preview at mobile, tablet, and desktop widths before publication.
...
A valid entry is discovered automatically at build time. Do not edit a central registry, reuse another entry's visual context, or add unrelated catalog application code.
