...
- **Orbit-arc sparkline:** each 12-point series is mapped along a semicircular arc (180°→0°, cresting over the top at 90°) with the value modulating the radius — a telemetry "orbit" rather than a straight polyline, with a dashed orbit track (bowed up) and an end-point dot. The arc sits entirely within its 120×56 viewBox. Stroke is green (up/favorable) or amber (down/unfavorable), matching the delta.
...
- Hover: KPI cards and buttons change state (KPI border→`--border-strong` + background→`--surface-2`; buttons background/border/color). The hover **state** is always available (it applies instantly under reduced-motion); only the **0.16s** `cubic-bezier(0.4, 0, 0.2, 1)` transition is gated behind `prefers-reduced-motion: no-preference`, matching the catalog convention.
