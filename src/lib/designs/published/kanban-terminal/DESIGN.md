...
- **Hover** is a multi-channel cue gated behind `(hover: hover)`: the static drag token `[..]` lifts to full opacity on card hover, and control cells brighten their border/text toward the accent yellow. No fill change, no transform — ncurses is flat. A tap on a touch device (`hover: none`) cannot leave a sticky state resembling selection.
