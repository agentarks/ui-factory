...
**Visual intent:** a frosted, translucent Kanban board floating over a soft, low-chroma cool-neutral wash ("mist") — calm and easy on the eye. Depth comes from layered backdrop blur, hairline edges, and soft shadow, never from heavy solid fills or saturated color. Light theme, comfortable density.
...
2. **Calm by default.** The field is a soft low-chroma wash, not a vibrant mesh — the glass reads through subtle tonal shifts, hairline edges, and soft shadow. Hue appears only on categorical label chips and avatars — and even there it is muted to low-chroma dusty tints, so no saturated color appears anywhere.
...
| Token             | Value                   | Role                                                           |
| ----------------- | ----------------------- | -------------------------------------------------------------- |
| `--ink`           | `oklch(0.26 0.025 240)` | Card titles, primary text                                      |
| `--ink-soft`      | `oklch(0.42 0.022 240)` | Meta: due dates, counts, subtitles, checklist                  |
| `--ink-faint`     | `oklch(0.46 0.022 240)` | Placeholder text, empty-state copy                             |
| `--accent-strong` | `oklch(0.52 0.06 235)`  | Active filter, focus ring, primary button                      |
| `--on-accent`     | `oklch(0.99 0.004 240)` | Text/borders on accent fills (tinted near-white, never `#fff`) |
| `--hair`          | `rgba(30,40,60,0.09)`   | Hairline glass edges                                           |

- **Neutrals are tinted** toward hue 240 (a faint cool blue-gray), never pure grey/black/white. `--on-accent` is a near-white, not `#fff`.
- **Accent gradients** (project chip, primary button): `linear-gradient(135deg, oklch(0.54 0.06 238), oklch(0.54 0.06 220))` — a muted dusty-slate, with a soft low-chroma glow.
- **Label tones** are muted: a very pale low-chroma OKLCH tint + a dusty mid-dark ink of the same hue, so contrast survives on glass and the palette stays calm. Example pairs:

  | Tone   | Background              | Ink                   |
  | ------ | ----------------------- | --------------------- |
  | violet | `oklch(0.93 0.025 290)` | `oklch(0.4 0.07 290)` |
  ...
  (Same muted pattern for teal, slate, indigo, pink, rose.) Ink sits near L 0.4–0.42 on a pale L≈0.93 tint — comfortably above 4.5:1 — while chroma stays ≤ 0.09 so chips read calm, not loud.

- **Avatars:** white initials on `oklch(0.46 0.05 <hue>)` — a muted dusty circle (chroma dropped to 0.05 for calm); the 0.46 lightness keeps white initials above 4.5:1. A 2px cool-gray rim (`rgba(30,40,60,0.14)` — white would vanish on the frosted surface) separates overlapping avatars.
...
A mesh of soft, very low-chroma radial blobs over a near-white diagonal base, painted on the board root:
...
- Z-depth is layered: the mist field sits at the back; frosted glass columns (`blur(18px)`) float above it; frosted cards (`blur(6px)`) float above the columns. Each layer is separated by its shadow, hairline border, and opacity, never by nested solid fills.
...
- **Label chip:** muted pale OKLCH tint + dusty dark ink (AA), one per concept; pill, ~0.66rem.
...
Rule of thumb: any new surface is a glass panel (header/column/card recipe at an appropriate radius) over the same mist field, with the same tokens, focus ring, and motion rules.
...
- **Use** when the product wants a calm, airy, modern surface that stays easy on the eye for long working sessions, and the target devices can afford `backdrop-filter` (modern evergreen browsers).
