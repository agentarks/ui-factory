...
| `--cyan`       | `64, 224, 232`           | RGB triplet used only inside `rgba()` glows (a data-URI SVG is not used, so a CSS variable cannot be referenced from an image) |
...
- **States demonstrated:** empty (In Review, `No cards yet`), loading skeleton (cyan-tinted blocks with a reduced-motion-gated opacity pulse), inline error (`Sync paused.` + Retry + dismiss, pale-red full border), static drag grip, done, priority, selected, hover (see motion guidance), keyboard focus, and reduced motion.
