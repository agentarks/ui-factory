...
1. **Surfaces match the canvas.** The neumorphic signature: raised cards, columns, and controls are the _same_ cool-gray color as the page background — on hover too; no background is ever recolored. Depth comes only from paired light/dark extrusion shadows, never from fills, gradients, or translucency. (On hover the lower-right extrusion shade shifts to a cool-cobalt highlight — a shadow change, not a fill; see Interaction and motion.)
...
| `--hover-shadow` | `4px 4px 12px rgba(38,72,168,.42), -3px -3px 7px rgba(248,250,254,.9)` | Cool-cobalt lower-right hover shadow + preserved near-light upper-left (ring re-applied alongside) |
...
- **Monochrome by default:** cards, columns, labels, controls, dividers, and dividers are all the cool-gray surface — large surfaces are never given a color fill, on hover or at rest. The only color in the system is a tiny set of **whisper-chroma semantic cues** (chroma 0.04–0.13), each always paired with a text label or icon, plus one **decorative** cool-cobalt hover shadow highlight (`--hover-shadow`): on hover the lower-right extrusion shade becomes cobalt while the surface itself stays neutral — never a background fill:
...
- Depth is layered by extrusion, not nested fills: the app bar is the most pronounced raise; columns and cards are `--raise-sm` resting (cards lift 1px on hover). Selected controls press _in_. Surfaces are always the canvas color — never a panel-on-panel tint or a hover background change (hover feedback is shadow-only; see below).
...
- Hover: the card lifts 1px; the primary lifts 1px; icon buttons and add-card darken their label. Active filter/view presses _into_ its track (`--press-sm` + `--ring-strong`). **Hover feedback is shadow-only** — no background or face is ever recolored. Hovering a task card or an appropriate raised secondary control (inactive filter chip, inactive Board/List button, the "more actions" icon, "Add a card") swaps its cast/extrusion shadow for a clearly contrasting **cool-cobalt lower-right shadow highlight** (`--hover-shadow`), while the **near-light upper-left shadow and the 1px hairline ring are preserved** (the ring is re-applied alongside `--hover-shadow`). **Selected/pressed controls, the primary, and the error Retry/dismiss actions are excluded** so their semantic styles win. The shadow highlight is **decorative feedback only** — the paired shadow, hairline ring, focus ring, and pressed/selected semantics all remain, so it is never the sole state signal. Native CSS only (no `:has()`, no filters). Text contrast is unaffected because no text-bearing background changes.
- Normal UI transitions (hover, focus, press, active-state shadow, and the cobalt hover shadow) are **0.16s ease**, gated behind `@media (prefers-reduced-motion: no-preference)`. Hover-only effects are additionally gated behind `@media (hover: hover)` so touch devices don't get sticky hover states. Under `prefers-reduced-motion` the cobalt shadow still applies (a shadow change is not motion) but **instantly**, with no fade; only the transform lifts and the fade are suppressed.
...
- Make every raised or pressed surface the **same cool-gray color as the canvas**; derive depth only from paired light/dark extrusion shadows. (On hover, only the cast shadow gains the documented cool-cobalt lower-right highlight — surfaces themselves stay neutral and are never recolored.)
...
- Don't colorize label chips or large surfaces; keep the board monochrome and let whisper-chroma live only in the tiny semantic cues (dots, priority, done, error), each paired with text. (The documented cool-cobalt hover shadow highlight is the one allowed exception — a cast-shadow color, never a background fill, never the sole state signal.)
...
- [ ] Cool-gray canvas (`oklch(0.9 0.006 255)`) sits behind every surface; raised cards/columns/controls use the **same** surface color and are never recolored on hover (hover feedback is the cobalt cast shadow only — no background change anywhere).
...
- [ ] All colors are OKLCH; neutrals tinted toward hue 255; no `#000`/`#fff`; no gradient text; no colored side-stripes; label chips and large surfaces are monochrome (no color fill, on hover or at rest); whisper-chroma lives only in stage dots/priority/done/error, each paired with text — except the single documented cool-cobalt hover shadow highlight (a cast-shadow color, not a surface fill).
...
- [ ] A cool-cobalt hover shadow highlight (`--hover-shadow`) swaps the cast/extrusion shadow of hovered task cards and raised secondary controls (inactive filter chips, inactive Board/List buttons, the "more actions" icon, "Add a card") to a clearly contrasting cobalt lower-right shadow, while the near-light upper-left shadow and the 1px hairline ring are preserved and **no background changes anywhere** (card/button/column/track faces all stay neutral). Decorative only: the shadow, hairline ring, focus ring, and pressed/selected semantics remain, so it is never the sole state signal; selected/pressed controls, the primary, and error actions are excluded; native CSS only; AA-dark text is unaffected (no text-bearing background changes); under `prefers-reduced-motion` the shadow applies instantly with no fade.
