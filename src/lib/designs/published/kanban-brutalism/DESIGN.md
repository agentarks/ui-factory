...
| `--grid`     | `oklch(0.82 0.006 250)`  | Graph-paper grid stroke (and skeleton block fill). The inline data-URI SVG serializes this token to the equivalent sRGB `#c1c4c8` = `rgb(193,196,200)` (verified to render the exact same pixels as the token), because a data-URI SVG cannot reference a CSS custom property. |
...
- **Graph-paper grid:** the canvas carries an inline, dependency-free SVG data-URI tiling a 16×16 grid stroked in `--grid` (serialized to sRGB `#c1c4c8` = `rgb(193,196,200)`, verified pixel-equal to the token — see the color table), so the drafting field is literally foregrounded. Cards/headers are opaque `--surface` sheets sitting on the grid; the grid shows through in the board padding, column gutters, and between cards.
