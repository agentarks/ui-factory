<script lang="ts">
	...
	// ---- Flow-field data-reactive hook (the wow mechanic). ----
	// Particle drift speed subtly scales with the Active-users delta. The
	// dashboard's most alive metric drives the calm motion behind the data.
	const usersKpi = kpis.find((k) => k.id === 'users') ?? kpis[1];
	// Numeric percent parsed from the locked fixtures ("3.1").
	export const ACTIVE_DELTA = parseFloat(usersKpi.delta); // 3.1 (up, favorable)
	// Gentle multiplier clamped to a calm band so the field never races.
	const FLOW_SPEED = 1 + Math.min(Math.max(ACTIVE_DELTA / 10, 0), 1.5); // 1.31
	...
	// ---- Generative flow-field canvas (decorative only; reduced-motion hidden) ----
	let canvasEl: HTMLCanvasElement | undefined = $state();
	$effect(() => {
		const canvas = canvasEl;
		if (!canvas) return;
		// Reduced motion: the canvas is hidden via CSS; never start the loop.
		...
		// Particle count scales with area, bounded for performance.
		const N = Math.min(190, Math.max(80, Math.round((w * h) / 8500)));
		...
		// Smooth pseudo-flow-field angle: layered trig, no dependencies.
		const flow = (x: number, y: number, t: number) =>
			(Math.sin(x * 0.0022 + t * 0.7) +
				Math.cos(y * 0.0022 - t * 0.5) +
				Math.sin((x + y) * 0.0014 + t * 0.3)) *
			Math.PI;

		// Drift speed carries the active-users delta (the data-reactive hook).
		const speed = 0.42 * FLOW_SPEED;
		let t = 0;
		let raf = 0;
		const draw = () => {
			...
			ctx.lineWidth = 1;
			ctx.strokeStyle = 'oklch(0.54 0.1 280 / 0.16)';
			ctx.lineCap = 'round';
			for (const p of particles) {
				...
				p.y = ny;
				p.life += 1;
				if (p.x < -12 || p.x > w + 12 || p.y < -12 || p.y > h + 12 || p.life > 360) {
					p.x = Math.random() * w;
					p.y = Math.random() * h;
					p.life = 0;
				}
...
<div class="dash-root" data-active-delta={ACTIVE_DELTA} data-flow-speed={FLOW_SPEED.toFixed(2)}>
	<canvas
		bind:this={canvasEl}
		class="flow"
		aria-hidden="true"
		data-flowfield="dashboard-saas-flowfield"
	></canvas>
			...
			<div class="grid-sub">
				...
				<aside class="rail" aria-label="Additional metrics">
					<div class="mini goal">
						...
						<span class="goal-note">On track — 2 weeks remaining.</span>
					</div>
					<div class="mini">
...
<style>
	...
	.dash-root {
		/* Flow-field tokens — OKLCH throughout, warm tinted paper + one indigo accent. */
		--canvas: oklch(0.975 0.008 75);
		--surface: oklch(0.992 0.005 75);
		--surface-2: oklch(0.98 0.006 75);
		--surface-3: oklch(0.952 0.009 75);
		--border: oklch(0.905 0.008 75);
		--border-strong: oklch(0.86 0.012 75);
		--text: oklch(0.24 0.012 75);
		--muted: oklch(0.45 0.014 75);
		--faint: oklch(0.49 0.014 75);
		--accent: oklch(0.5 0.12 280);
		--accent-ink: oklch(0.4 0.12 280); /* AA-safe indigo for text & primary fills */
		--pos: oklch(0.42 0.11 155);
		--neg: oklch(0.4 0.14 25);
		--on-accent: oklch(0.99 0.004 75);
		--chart-grid: oklch(0.885 0.009 75);
		--chart-users: oklch(0.57 0.014 75);
		--track: oklch(0.91 0.009 75);
		--err-bg: oklch(0.95 0.022 25);
		--err-border: oklch(0.74 0.1 25);
		--spark-up: oklch(0.42 0.11 155);
	...
	/* ---------- Generative flow-field canvas (decorative, behind everything) ---------- */
	.flow {
		position: absolute;
		inset: 0;
		z-index: 0;
	...
	/* ---------- App bar (opaque warm paper, controls on the light surface) ---------- */
	.appbar {
		position: sticky;
		top: 0;
		z-index: 40;
	...
	.btn-primary:hover {
		background: oklch(0.38 0.12 280);
	}
	...
	.sw2 {
		background: oklch(0.55 0.09 300);
	}

	.sw3 {
		background: oklch(0.68 0.012 75);
	}

	.sw4 {
		background: oklch(0.82 0.01 75);
	}
	...
	.donut .s2 {
		stroke: oklch(0.55 0.09 300);
	}

	.donut .s3 {
		stroke: oklch(0.68 0.012 75);
	}
	...
	/* ---------- Reduced motion: hide the flow-field canvas entirely ---------- */
